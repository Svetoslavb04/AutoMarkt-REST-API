...
### Login
Log in by sending a `POST` request with email and password to `/login`. The service will respond with an object, containing user information `{ _id, email, username }` and sets two cookies: one with the **refresh token** and one for a **access token** with short lifetime.
